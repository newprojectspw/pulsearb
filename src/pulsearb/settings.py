...
class RecorderSettings(BaseModel):
    ...
    max_tokens_assinados: int | None = Field(default=None, ge=2)
    ...
    bytes_por_hora_estimados: float = Field(default=2_000_000_000.0, gt=0)
    ...
    margem_de_disco: float = Field(default=1.2, ge=1)
    ...
    duracao_minima_para_preflight_s: float = Field(default=6 * 3600.0, gt=0)

    @field_validator("max_tokens_assinados")
    @classmethod
    def _limite_de_tokens_deve_ser_par(cls, value: int | None) -> int | None:
        """O escopo assina janelas inteiras (Up + Down).

        Um limite ímpar não consegue assinar uma janela a mais e deixaria
        capacidade do teto inutilizada; recusá-lo no carregamento torna a
        decisão explícita em vez de produzir um escopo menor em silêncio.
        """
        if value is not None and value % 2:
            raise ValueError(
                "recorder.max_tokens_assinados deve ser par porque cada janela "
