...
| 2 | `anchor_sweep` com `window_seconds=30` vs `60` nas janelas de 5m da M2_72H; datar a nota do API_NOTES | Mac | 1 execução | 2.2 — **✅ 2026-09-18: a varredura τ por duração (v2, mesmo caminho do backtest) dá 5m 0,9994 (1 discordante em 1.764), 15m 1,0, 4h 1,0 com o stream de 60 s.** A v1 media pela média das amostras (a definição que o M2.6 já mostrou perder) e foi descartada. Nota do §12.3 datada com a tabela; engine fica com 60 s. Prova directa com o stream de 30 s fica opcional (`feeds.rtds_assinar_twap_thirty`, #156) |
