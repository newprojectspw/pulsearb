...
| **4.0** | **Motor MAKER — a rota que sobrou** | 🟡 **duas peças de quatro, em 2026-08-31.** ✅ **(a) onde cotar:** `live/cotacao.py` escolhe `distancia_ticks` e tamanho pelo líquido `rewards − markout`, usando a **mesma** `score_de_nivel` do backtest; publica as parcelas separadas porque uma é estimativa com hipótese de fila e a outra é medida (1.7); 20 testes. ✅ **(b) `orderType` configurável:** `ClienteDeOrdens(tipo_de_ordem="GTC")`, com tipo desconhecido falhando na **construção** e não no envio (`[VERIFICADO]` §4.1); default segue FOK. 🟡 **(c) repousar:** `live/repouso.py` decide *mexer ou deixar* com **histerese dupla** — piso de ganho (0,50 USDC) e tempo mínimo repousada (30 s) —, porque reposicionar custa a fila e o livro pisca; cotação que **deixa de pontuar** vence as duas travas, já que ficar seria pagar risco de execução por zero reward. Toda decisão sai com **motivo nomeado**, como no `risk/gates.py`. 17 testes. **Ligado em 2026-09-06:** `live/execucao_maker.py` (`aplicar_decisao`, 10 testes) traduz a `Decisao` do `repouso` em `cliente.enviar`/`cliente.cancelar`, com o ciclo do `order_id` do servidor. A trava que dá nome ao módulo: reposicionar é cancelar E DEPOIS enviar, e **cancelamento incerto PARA a sequência** — enviar a nova por cima da antiga que talvez repouse seria posição dupla entre dois makers (verificado por mutação). **Reconciliação em 2026-09-06:** `reconciliar` casa o `GET /data/orders` (§4.5) com o que o nosso lado esperava e separa **órfãs** (no servidor, não esperadas — o envio que ficou INCERTA e afinal entrou) de **fantasmas** (esperadas, sumidas do servidor); `cancelar_orfas` tira as órfãs do livro, e loga a meio-preenchida como posição real a reconciliar no PnL. Leitura fail-closed: erro ao ler SOBE, não vira livro limpo. 15 (4.0(c) — execução maker) + as 68 do cliente, o central verificado por mutação nos dois níveis. **Só falta do 4.0(c):** o LAÇO que chama isto a cada tick — hoje as peças existem e se compõem, mas nada em `live/shadow.py` as orquestra ainda. ⬜ **(d) posição na fila:** o WS agregado não mostra, e é dela que dependem os 3 termos que travam o 1.6 |
