...
| 0.8 | 72 h contínuas e limpas | 🟡 **contínuas: sim. Limpas: 101 silêncios > 30 s — falta a barra escrita** | **MEDIDO em 2026-09-17** (`jq '.gravacao.silencio_do_rtds'` sobre `relatorios/M2_72H_20260912_ok.json`): `silencios: 101`, `total_s: 614,25` (0,24 % dos 259.200 s), `maior_s: 83,04`, `por_escopo: topico_do_ativo 99 / conexao_inteira 2` (74,01 s), `suspeita_de_assinatura_caducada: 83`. **Contínuas ✅:** janela exacta de 72 h, 72 arquivos, 287.745.263 registros, 0 corrompidas; as 2 quedas da conexão inteira somam 74 s — o keepalive do 0.6 segurou. **Limpas — nem ✅ nem ❌, porque a barra nunca foi escrita:** o 0.7 fechou com `silencios: 0` numa hora; em 72 h a contagem é 101 e ninguém definiu quanto silêncio uma gravação de 72 h pode ter. Dois factos pesam contra: (a) o maior silêncio (83 s) é MAIOR que a janela do TWAP (60 s) — uma janela de 5m que fechasse dentro dele teria o preço de liquidação em branco; (b) 83 dos 99 silêncios de tópico levam a marca `suspeita_de_assinatura_caducada` (conexão viva, tópico calado) — o mesmo defeito que o 0.5 mediu (2.482 reassinaturas sem efeito) e o 0.6 escalou. Um facto pesa a favor: cobertura 99,8 % e 0,24 % de silêncio total. **O que falta para fechar, escrito:** (1) Paulo decide a barra e ela entra nesta linha — proposta: `total_s ≤ 1 %` E `maior_s ≤ 60 s` (nenhum silêncio mais longo que a janela do TWAP) E `conexao_inteira ≤ 2`; por esta proposta a gravação REPROVA em (a); (2) **MEDIDO em 2026-09-18** (`ancora.lacunas_do_stream` do mesmo relatório; lacuna = sem amostra do TWAP nos 10 s anteriores): de 2.318 janelas alimentadas, **8 fecharam dentro de uma lacuna (0,35 %)** e 14 abriram numa; 142 são de ativo sem stream gravado, não contam. Ou seja, o custo dos 101 silêncios em resoluções é 8 janelas em 72 h — cada uma seria uma liquidação que o bot não saberia calcular pelo stream. Este número NÃO altera a barra proposta em (1), que foi registada antes de o ver; uma barra sobre o resultado (fechamentos em lacuna ≤ 0,5 %) passaria esta gravação, mas escolhê-la agora seria escolher a barra depois do número, e é Paulo quem decide; (3) se a barra reprovar, o conserto é no recorder — **e a causa ganhou nome em 2026-09-17:** o RTDS recusa a reassinatura com `statusCode 400 connection_id_fk` e o feed não lia (API_NOTES §6.2b, sub-item do 0.6); com a recusa lida e o socket derrubado na primeira, a PRÓXIMA gravação de 72 h é a que fecha o 0.8. Histórico: a linha dizia *'contador reiniciou 01:31 UTC'* de 23/08 a 12/09, e de 12/09 a 17/09 dizia que faltava só o número |
...
| # | Item | Estado |
|---|---|---|
| 5.1 | Carteira **dedicada**, só com o capital de operação em USDC na Polygon | 🟡 **metade executada em 2026-09-11 — a metade que não custa nada.** ✅ **carteira criada** (10/09) e ✅ **credenciais L2 derivadas** (11/09, `scripts/derivar_credenciais.py`, arquivo `0600`, segredo nunca impresso). A medida que destravou isso: o passo 6 **NÃO precisa de carteira fundeada** — o CLOB aceitou a assinatura L1 de uma carteira sem USDC, sem MATIC e sem allowance. Por isso o RUNBOOK foi reordenado: **6 e 7 antes de 3, 4 e 5**, porque descobrem de graça se a assinatura L1 é aceita, e travar no passo 6 *depois* de fundear seria dinheiro parado numa carteira quente esperando conserto. ⬜ **Falta o que move dinheiro:** fundear e aprovar as allowances de USDC e CTF — e é aí que está a armadilha que não avisa: sem elas a ordem é aceita pelo CLOB e falha na liquidação, com mensagem que não fala em allowance. Os endereços dos contratos vão ao API_NOTES `[VERIFICADO]` antes de qualquer transação. Procedimento completo em `RUNBOOK_VPS.md` §8.1: EOA (`signature_type=0`, API_NOTES §3), os 7 passos em ordem, e a armadilha que não avisa — **allowances de USDC e CTF setadas à mão antes da primeira ordem**, senão a ordem é aceita pelo CLOB e falha na liquidação, com mensagem que não fala em allowance. `CredenciaisL2.do_ambiente()` fecha o caminho das 4 variáveis L2 (antes só a chave privada tinha).  Os endereços dos contratos vão ao API_NOTES `[VERIFICADO]` antes de qualquer transação |
