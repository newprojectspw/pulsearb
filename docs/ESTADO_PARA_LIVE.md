...
**Rodada 4 (2026-09-14, 20 min, registro próprio, tetos do taker elevados
por env só para o ensaio: `STAKE_MAX_POR_TRADE_USDC=1000`,
`STAKE_MAX_POR_JANELA_USDC=1000`, `EXPOSICAO_MAX_USDC=100000`,
`POSICOES_MAX_ABERTAS=500`, `SPREAD_MAXIMO=0.10`): houve cotação-sombra
...
mesmo commit deste parágrafo: o preço sai do `livro.mid` da passada (sem
meio, `livro_sem_meio` e não cota), `Cotacao.preco` arredonda para a grade
do tick (bid para baixo, ask para cima — o estimado e o colocado olham o
mesmo preço), e as candidatas são `dois_lados=False`, para a conta
descrever a ordem que existe. Testes em `tests/test_4_0c_laco_maker.py`
(`TestOPrecoSegueOMeioDoLivro`) e `tests/test_m4_cotacao_maker.py`
(`TestOPrecoCaiNaGradeDoTick`). 🟡 falta: (a) cotar os DOIS lados — bid no
Up e bid no Down —, que exige duas ordens por janela no `execucao_maker` e
triplica o score em faixa; (b) rodar SHADOW de novo com o preço consertado
e conferir no diário que `preco_limite` acompanha o meio de cada mercado;
(c) os tetos do taker (`stake_max_por_trade_usdc` 5, `spread_maximo` 0,04)
barram qualquer cotação de 1.000 shares — a rota maker precisa de tetos
próprios em `Settings`, não de env por rodada.
