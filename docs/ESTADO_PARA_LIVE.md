...
`tests/test_m4_cotacao_maker.py` (`TestOPrecoCaiNaGradeDoTick`). 🟡 falta:
(b) rodar SHADOW de novo com as duas pernas e conferir no diário que cada
janela tem UM `cotacao_colocada` por token e que `preco_limite` acompanha
o meio de cada mercado; (c) os tetos do taker (`stake_max_por_trade_usdc`
5, `spread_maximo` 0,04) barram qualquer cotação de 1.000 shares — o
operador que quiser a rota maker tem de subir os tetos de propósito
(`PULSEARB_RISK__*`), com o capital que os pools exigem
(`rewards_min_size` 50–1.000 shares × preço); o bot não os sobe sozinho.
