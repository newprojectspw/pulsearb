...
| 3.13 | SHADOW rodando 24 h sem crash | 🟡 **reiniciado em 2026-08-31** após fix de `relogio_derivado`: `_portao_do_relogio` recusava em SHADOW por latência estrutural de rede (~1278 ms Polymarket), apagando 100 % do diário. Corrigido em `gates.py` — só LIVE recusa por `relogio_derivado`. Shadow em `caffeinate -i python -m pulsearb.live.shadow --duration 24h --curva-de-variancia relatorios/VARIANCIA_23AGO.json`. Falta completar as 24 h e conferir o diário (espera-se `edge_abaixo_do_threshold` dominante) |
