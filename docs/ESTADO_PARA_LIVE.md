...
| 3.13 | SHADOW rodando 24 h sem crash | 🟡 **rodando desde ~2026-08-30 23:30 UTC** — `caffeinate -i python -m pulsearb.live.shadow --duration 24h --curva-de-variancia relatorios/VARIANCIA_23AGO.json`, PID 24507, `feeds_saudaveis=True`, 1244 tentativas, `edge_abaixo_do_threshold` (esperado: o taker reprova 1.1/1.4). O `replay/ao_vivo.py` já fechou o **mesmo caminho** — séries E18 do ciclo e do backtest byte a byte idênticas sobre amostra real. Falta completar as 24 h e conferir o diário |
