...
| **1.1 / 1.4 ausência de borda** (as cinco bandas negativas, de −22,54 a −113,64; irrestrito −67,27, `hit_rate` 0,4172) | resultado de **medição**, não defeito | **o instrumento passou a existir em 2026-08-31 — falta RODAR.** `direcao_sem_fill` mede a acurácia da direção sobre **todos** os instantes que cruzaram o threshold, preenchidos ou não, com p-valor binomial bilateral. É a coorte fixa que a §2d-ter pedia: há teste travando que ela **não muda entre 150 ms e 1000 ms**, enquanto o `hit_rate` muda. Lê-se em `backtest.direcao_sem_fill.por_janela`, e o `resumo_m2.py` imprime. **O que ele decide:** direção ≈ 0,5 ⇒ não há sinal, e o taker encerra no registro; direção ≠ 0,5 com p < 0,05 ⇒ há informação, e a reprovação é de execução/capacidade — conserto diferente. Não é critério novo: os dez foram escritos antes dos números |
