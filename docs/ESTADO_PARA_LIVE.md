...
| 3.13 | SHADOW rodando 24 h sem crash | ✅ **CUMPRIDO em 2026-08-31** — PID 26423: **29,07 h de diário contínuo, sem crash**. 64.319 linhas, **295 intenções aprovadas**, **cinco portões exercitados** (`pausa_por_sequencia` 62.811 · `preco_fora_da_faixa` 543 · `spread_anomalo` 525 · `livro_desconhecido` 139 · `feed_parado` 6), disjuntor desarmado. **O PnL de +101,92 NÃO conta** — rodou com o motor que gravava `preco_pago = best_ask`, corrigido só depois; ver o aviso abaixo e o 4.2. **As 29,07 h para um `--duration 24h` são a medição do 3.14 em produção:** 5,07 h a mais, que é o que a máquina dormiu |
| **3.14** | **`--duration` não encerrava: o relógio congela quando a máquina dorme** | ✅ **causa medida e corrigida em 2026-08-31.** O ensaio do 3.13 seguiu vivo em **24,6 h** com `--duration 24h`. Não era cancelamento preso — escrevi o teste de `run()` com descoberta lenta e ele **passou**. A causa é outra: `time.monotonic()` no macOS sai de `mach_absolute_time()`, que **congela em suspensão**. Medido nesta máquina: **190,8 h de monotonic contra 370,8 h de parede desde o boot — 180 h de sono**. O deadline era só monotônico, então `24h` virava 24 h + o que a máquina dormisse. `caffeinate -i` não cobre: impede o sono por inatividade, não o de tampa fechada. **Corrigido com prazo DUPLO** (`prazo_vencido`), encerrando no primeiro a vencer — só parede também não serviria, porque um ajuste de NTP encurtaria a rodada, e é o relógio que o 3.10 diz não poder assumir estável. 3 testes novos, um por modo de falha. **A medição em produção fecha o caso:** o ensaio do 3.13 pediu 24 h e entregou **29,07 h de diário** — as 5,07 h de diferença são o sono da máquina, e é o número que o conserto elimina |
...
| 4.2 | **SHADOW ≥ 2 semanas** com edge líquido *medido* | ⬜ **o relógio precisa recomeçar.** A tentativa de 01/09 (PID 56001) encerrou em **13,86 h de 24 h**, limpa, com `falhou: null` e sem uma linha explicando — ver 3.15. Corrigido; o próximo ensaio é o que conta. E o 4.2 exige *edge líquido medido*, que com 1.1/1.4/1.6 reprovados não deve aparecer: o ensaio serve para **medir isso ao vivo**, não para esperar que mude |
| **3.15** | **Rodada encerrava cedo e engolia o motivo** | ✅ **achado e corrigido em 2026-09-02.** O ensaio do 4.2 parou em 13,86 h de 24 h **sem erro visível**. Causa: `laco_de_relato` não tinha `try/except`, e `run()` usa `FIRST_COMPLETED` — que trata *"tarefa morreu com exceção"* igual a *"tarefa terminou"*. Uma exceção no relato encerrava os três laços, e o `gather(return_exceptions=True)` do `finally` **engolia o motivo**: saía o JSON final normal, `falhou: null`, nada no log. Dois consertos: o relato passou a tolerar erro (relato é observação, não é a rodada) e o `run()` passou a **inspecionar** os resultados do `gather`, anotando em `falhou` a primeira tarefa que morreu — ignorando `CancelledError`, que é o caminho normal. 3 testes, conferidos pelo avesso |
