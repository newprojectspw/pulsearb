...
class TestAFatiaSaiNoRelato:
    ...
    def test_a_contagem_nao_responde_de_onde_veio_o_numero(self):
        """12% das passadas podem ser 80% do resultado.

        `passadas_quase_inteiras` diz quantas vezes o modelo deu o pool
        inteiro; não diz quanto do reward veio delas. Uma passada com fatia
        1,0 num pool de `daily_rate` alto pesa muito mais que a sua fração na
        contagem — e é a contribuição, não a contagem, que separa "o 4.2 mede
        estratégia" de "o 4.2 mede mercado deserto".
        """
        caixa = CaixaDoMaker()
        aberta = _aberta(desde=1000.0)

        # deserto: fatia 1,0, e o pool paga o mesmo que os outros
        caixa.acertar(
            "j", aberta, _livro_de_profundidade(1.0), PARAMS, agora_epoch=1010.0
        )
        pro_rata_do_deserto = caixa.rewards_pro_rata_usdc

        # disputado: fatia minúscula, e MUITO mais tempo repousando
        caixa.acertar(
            "j",
            aberta,
            _livro_de_profundidade(5000.0, niveis=2),
        ...
        fatia = caixa.resumo()["fracao_do_pool"]
        assert fatia["rewards_dessas_passadas"] == pytest.approx(
            pro_rata_do_deserto, abs=1e-4
        )
        assert fatia["fracao_do_total_que_vem_delas"] == pytest.approx(
            pro_rata_do_deserto / caixa.rewards_pro_rata_usdc, abs=1e-4
        )
        # E o ponto do teste: a contagem diz "metade das passadas", a
        # contribuição diz outra coisa. Se as duas batessem, este campo não
        # precisaria existir.
        assert fatia["passadas_quase_inteiras"] == 1
        assert fatia["fracao_do_total_que_vem_delas"] != pytest.approx(0.5, abs=0.05)

    def test_sem_reward_nenhum_a_contribuicao_e_None(self):
        """Zero diria "nada veio de mercado deserto". `None` diz "não medi"."""
        fatia = CaixaDoMaker().resumo()["fracao_do_pool"]
        assert fatia["fracao_do_total_que_vem_delas"] is None
        assert fatia["rewards_dessas_passadas"] == 0.0
